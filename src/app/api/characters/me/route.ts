...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (campaignId instanceof Response) return campaignId;
    const agent = await requireSingleCampaignAgent(req, campaignId);
    if (agent.role !== "player") return new Response("Player role required", { status: 403 });
    ...
    if (!session) return new Response("Campaign session not found", { status: 404 });
    ...
    if (existing) {
      ...
      if (existing.type !== expectedType || existing.idempotencyHash !== idempotencyHash) {
        return new Response("Idempotency-Key was already used with a different request", { status: 409 });
      }
